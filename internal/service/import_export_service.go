...
type ImportExportService interface {
	ExportNote(rail flow.Rail, note *domain.Note, path string) error
...
func (s *ImportExportServiceImpl) ExportNote(rail flow.Rail, note *domain.Note, path string) error {
	if note == nil {
		return fmt.Errorf("note cannot be nil")
	}

	rail.Infof("Exporting note: %s", note.ID)
...
func (s *ImportExportServiceImpl) ExportNotes(rail flow.Rail, notes []*domain.Note, dir string) error {
	if len(notes) == 0 {
	...
	rail.Infof("Exporting %d notes to: %s", len(notes), dir)
	...
	for _, note := range notes {
		...
		err := s.ExportNote(rail, note, path)
		if err != nil {
...
func (s *ImportExportServiceImpl) ImportNote(rail flow.Rail, path string, onDuplicate func(note *domain.Note) bool) (*domain.Note, error) {
	if path == "" {
		return nil, fmt.Errorf("file path cannot be empty")
	}

	rail.Infof("Importing note from: %s", path)
...
func (s *ImportExportServiceImpl) ImportNotes(rail flow.Rail, dir string, onDuplicate func(note *domain.Note) bool) ([]*domain.Note, error) {
	if dir == "" {
		return nil, fmt.Errorf("directory path cannot be empty")
	}

	rail.Infof("Importing notes from: %s", dir)
	...
	for _, file := range files {
		...
		note, err := s.ImportNote(rail, path, onDuplicate)
		if err != nil {
